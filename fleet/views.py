...
def dashboard(request):
    context = {
        ...
        'vehicles_in_maintenance': Vehicle.objects.filter(status='IN_SHOP').count(),
...
def trip_dispatch(request, pk):
    ...
    messages.success(request, f'Trip dispatched. {trip.vehicle} and {trip.driver} are now On Trip.')
    return redirect('trip_list')
...
def trip_cancel(request, pk):
    trip = get_object_or_404(Trip, pk=pk)
    if trip.status != 'DISPATCHED':
        messages.error(request, 'Only Dispatched trips can be cancelled this way.')
        return redirect('trip_list')
    ...
    messages.success(request, 'Trip cancelled. Vehicle and Driver restored to Available.')
    return redirect('trip_list')
...
def maintenance_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            log.vehicle.save()
            messages.success(request, f'Maintenance logged. {log.vehicle} is now In Shop.')
...
def maintenance_close(request, pk):
    ...
    messages.success(request, f'Maintenance closed. {log.vehicle} is Available again.')
    return redirect('maintenance_list')
